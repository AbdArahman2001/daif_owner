...
import '../../../../utill/values_manager.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<BookingsController>(builder: (controller) {
      return WillPopScope(
        ...
        child: Scaffold(
          appBar: CustomAppBar(
            title: isEditMode ? locale.update_booking_info : locale.new_booking,
            actions: [SizedBox.shrink()],
            leading: IconButton(
              onPressed: () async {
                if (!controller.isDataEdited) {
                  Get.back();
                } else {
                  final bool result = await showDialog(
                      context: context,
                ...
                child: Column(
                  ...
                  children: [
                              ...
                              Container(
                                ...
                                child: DropdownButton<BookingPeriod>(
                                    ...
                                    items: BookingPeriod.values
                                        .map((period) =>
                                            DropdownMenuItem<BookingPeriod>(
                                                alignment: Alignment.centerLeft,
                                                value: period,
                                                child: Text(
                                    ...
                                    onChanged: controller
                                        .changeSelectedPeriodWithoutGetDate),
                              ),
                            ],
                          ),
                        ]),
                    ...
                    TextFormField(
                      ...
                      decoration:
                          const InputDecoration(hintText: "Mohammed Ahmed"),
                    ),
                    SizedBox(
                      height: 16.h,
                    ),
                    _SingleStructure(leftChildren: [
                      Text(
                        locale.phone_number,
                        style: style_400_16(ColorManager.blackTextColor),
                      ),
                      ...
                      TextFormField(
                        keyboardType: TextInputType.number,
                        validator: (input)=>Validators.identityNumberValidator(input, context),
                        onChanged: controller.raiseEditingFlag,
                    ...
                    _SingleStructure(leftChildren: [
                      ...
                      TextFormField(
                        keyboardType: TextInputType.number,
                        validator: (input)=>Validators.numValidator(input, context),
                        onChanged: controller.raiseEditingFlag,
                    ...
                    TextFormField(
                      keyboardType: TextInputType.number,
                      validator: (input)=>Validators.intValidator(input, context),
                      onChanged: controller.raiseEditingFlag,
                    ...
                    CustomElevatedButton(
                        onPressed: () async {
                          if (isEditMode) {
                            controller
                                .updateBookingWithItsAttachments(bookingId!,context);
                          } else {
                            await controller.createNewBookingWithItsAttachments(context);
                          }
...
class _SingleStructure extends StatelessWidget {
  const _SingleStructure(
      {Key? key, required this.leftChildren, required this.rightChildren})
      : super(key: key);
